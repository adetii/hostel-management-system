...
async function generatePdf() {
  ...
  const styles = StyleSheet.create({
    ...
    bookingCard: { borderWidth: 1, borderStyle: 'solid', borderColor: '#ddd', borderRadius: 4, padding: 8, marginTop: 8 },
    divider: { height: 1, backgroundColor: '#eee', marginTop: 6, marginBottom: 6 },
    watermarkContainer: {
      position: 'absolute',
    ...
    watermarkText: { fontSize: 64, color: '#000', textAlign: 'center' },
    footer: { position: 'absolute', left: 24, right: 24, bottom: 16, textAlign: 'center' },
    footerText: { fontSize: 9, color: '#000' },
    orgName: { fontSize: 14, fontWeight: 'bold', marginBottom: 2, textAlign: 'center', color: '#000' }
  });
  ...
  function StudentProfileDocument({ student, bookings = [], branding = {} }) {
    return h(
      ...
      h(
        ...
        { size: 'A4', style: styles.page },
        h(Watermark, { branding }),
        h(PdfHeader, { title: 'Student Profile', branding }),
  ...
  async function streamPdf(res, element, filename = 'export.pdf') {
    try {
      ...
      if (typeof renderToBuffer === 'function') {
        const out = await renderToBuffer(element); // typically Uint8Array
        buf = Buffer.from(out);
      } else {
        // Fallback to old API
        const data = await pdf(element).toBuffer();
        if (Buffer.isBuffer(data)) {
          buf = data;
        } else if (data && (data.byteLength !== undefined || data.length !== undefined || data.buffer)) {
          // Handles Uint8Array/ArrayBuffer/Array-like
          buf = data.buffer ? Buffer.from(data.buffer) : Buffer.from(data);
        } else {
          throw new Error('PDF buffer generation failed: unknown return type from toBuffer()');
        }
      }
  ...
  // Return the components and functions that need to be used
  return {
    StudentsDocument,
...
// Export functions that use the dynamic import
const exportStudentsPdf = async (req, res) => {
  try {
    const { StudentsDocument, streamPdf, React } = await generatePdf();
...
const exportSingleStudentPdf = async (req, res) => {
  try {
    const { StudentProfileDocument, streamPdf, React } = await generatePdf();
...
const exportBookingsPdf = async (req, res) => {
  try {
    const { BookingsDocument, streamPdf, React } = await generatePdf();
